...
const DashCarousel = () => {
  ...
  const handleFormSubmit = async (event) => {
    ...
    setload(true);
    let response = await fetch("http://http://34.131.12.7/api/carousel/addslide", {
      method: "POST",
      body: bodyContent,
      headers: headersList,
    });
  ...
  const handleDelete = async (e) => {
    ...
    if(slideselected[0]===null){//only one slide is there
      ...
      let response = await fetch(
        "http://http://34.131.12.7/api/carousel/deleteslidebytitle",
        {
          method: "POST",
          headers: headersList,
          body: bodycontent,
  ...
  useEffect(() => {
    ...
    fetch("http://http://34.131.12.7/api/carousel/getslides")
                ...
                <div className=" row justify-content-center">
                  {images.map((e) => (
                    <div key={e._id} className=" col-md-12  mb-3">
                      <img
                        src={"http://http://34.131.12.7" + e.image.path}
                        alt={`${e.title}`}
