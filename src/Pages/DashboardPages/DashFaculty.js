...
const DashFaculty = () => {
  ...
  const handleFormSubmit = async (event) => {
    ...
    let response = await fetch("http://34.131.12.7/api/faculty/addfaculty", {
      method: "POST",
      body: bodyContent,
      headers: headersList,
    });
  ...
  const handleDelete = async (e) => {
    ...
    facultyselected.map(async (dtitle) => {
      ...
      let response = await fetch(
        "http://34.131.12.7/api/faculty/deletefaculty",
        {
          method: "POST",
          headers: headersList,
          body: bodycontent,
  ...
  useEffect(() => {
    ...
    fetch("http://34.131.12.7/api/faculty/getfaculty")
