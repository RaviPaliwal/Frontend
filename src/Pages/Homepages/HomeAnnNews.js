...
const HomeAnnNews = () => {
  ...
  useEffect(() => {
      const fetchNews = async () => {
        const response = await fetch('http://http://34.131.12.7/api/news/getall');
  ...
  useEffect(() => {
    const getRecAnn = async () => {
      ...
      let response = await fetch(`http://http://34.131.12.7/api/announcement/getbymonth-year/${date()}`, {
        method: 'GET',
        headers: headersList,
      });
  ...
  {news.slice(0, 6).map((item) => (
    <Newsitem
      ...
      imageUrl={item.image && "http://http://34.131.12.7"+item.image.path}
